...
public:
  ...
  {
    this->declare_parameter<double>("publish_frequency", 2.0);  // Default frequency: 2.0 Hz
    double publish_frequency = this->get_parameter("publish_frequency").as_double();

    publisher_ = this->create_publisher<std_msgs::msg::String>("topic", 10);
    timer_ = this->create_wall_timer(
      std::chrono::duration<double>(1.0 / publish_frequency), std::bind(&MinimalPublisher::timer_callback, this));
    service_ = this->create_service<cpp_pubsub::srv::ModifyMessage>(
    "modify_message", std::bind(&MinimalPublisher::modifyMessageCallback, this, _1, _2)
  );
   ...
   // Use DEBUG logging level
    ...
    RCLCPP_FATAL_STREAM(this->get_logger(), "Fatal error message in constructor");
...
)
{
  ...
  // Use DEBUG logging level
  RCLCPP_DEBUG_STREAM(this->get_logger(), "Debug message in modifyMessageCallback");

  // Use ERROR logging level
  RCLCPP_ERROR_STREAM(this->get_logger(), "Error message in modifyMessageCallback");

  // Use FATAL logging level
  RCLCPP_FATAL_STREAM(this->get_logger(), "Fatal error message in modifyMessageCallback");

  // Use INFO logging level
  RCLCPP_INFO_STREAM(this->get_logger(), "Message modified: '" << message.data << "'");
...
int main(int argc, char * argv[])
{
  ...
  // Use INFO logging level
  RCLCPP_INFO(rclcpp::get_logger("minimal_publisher_main"), "Starting MinimalPublisher node");

  rclcpp::spin(std::make_shared<MinimalPublisher>());

  // Use INFO logging level
  RCLCPP_INFO(rclcpp::get_logger("minimal_publisher_main"), "Shutting down MinimalPublisher node");
